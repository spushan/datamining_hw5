...
for i in rules.index:
    consequent_sc = ohe_df[rules['consequents'][i]].value_counts()[1]
    prior = consequent_sc/len(encoded_vals)
    
    print(f"{rules['antecedents'][i]} -> {rules['consequents'][i]}")
    print(f"Support: {rules['support'][i]}")
    print(f"Confidence: {rules['confidence'][i]}")
    print(f"Prior: {prior}")
    print(f"Gain in Confidence: {100*((rules['confidence'][i]-prior)/prior)}")
    print()
#Meat, Cheese -> Eggs
